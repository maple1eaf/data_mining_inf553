...
class UndirectedGraph:
    ...
    def countModularityBySelf(self):
        groups = self.getConnectedGraphs()
        self.freshDegrees()
        modularity_sum = 0
        for group in groups:
            sum_group = 0
            for vname1 in group:
                for vname2 in group:
                    ...
                    degree_expect = degreei * degreej / (2 * self.num_edge)
                    sum_group += Aij - degree_expect
            modularity_sum += sum_group
        modularity = modularity_sum / (2 * self.num_edge)
        return modularity

    def countModularityDependOnOriginalGraph(self, original_graph):
        ori = original_graph
        m = ori.num_edge
        ori.freshDegrees()
        ...
        for group in groups:
            ...
            for vname1 in group:
                for vname2 in group:
                    Aij = ori.getWeight(vname1, vname2)
                    degreei = ori.getDegree(vname1)
                    degreej = ori.getDegree(vname2)
